...
Bio::Rfam::MotifIO - a module that enables access to the Rfam SVN for motifs

=cut

package Bio::Rfam::MotifIO;
...
sub loadRfamMotifFromLocalFile {
  my ( $self, $motif, $dir, $source ) = @_;

  unless ( -d "$dir/$motif" ) {
    confess("Could not find motif directory $dir/$motif");
  }
  ...
  foreach my $path (@files) {
    my $f = basename($path);
    if ($f eq "CM" or $f eq "DESC" or $f eq "SEED"){
    unless ( -e "$dir/$motif/$f" ) {
      confess("Could not find $dir/$motif/$f\n");
    }
    my $fh;

    open( $fh, "$dir/$motif/$f" )
      or confess("Could not open $dir/$motif/$f:[$!]");
    $params{$f} = $fh;
  }}
...
sub parseCM {
  ...
  my $cmObj = 'Bio::Rfam::Motif::CM'->new($cm);
  return ($cmObj);
}
...
#         then we validate it after all DESC lines have been parsed
    ...
    #Database cross references
      ...
      for ( ; $i <= $#file ; $i++ ) {
        ...
        #snornadb; snR49
          ...
          print( "Bad reference Line: unknown database |$file[$i]|.\n"
  ...
  my $desc = 'Bio::Rfam::Motif::DESC'->new(%params);
  return $desc;
