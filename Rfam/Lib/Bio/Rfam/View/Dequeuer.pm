...
=head2 _submit_slurm_job

Given the job spec generated by L<_build_job_spec>, this method builds the 
shell command needed to run the view process and submits it to slurm using
sbatch. Return value is the job id of the submitted job.
...
sub _submit_slurm_job {
  ...
  $self->_log->debug( "building Slurm job command" );

  my $working_dir =   $job_spec->{tmp_dir} . '/' 
                    . $job_spec->{lsf_user} . '/' 
                    . $job_spec->{job_id};
  $self->_log->debug( "Slurm working directory: $working_dir" );
  ...
  my $slurm_command =   "mkdir -p $working_dir"
                    . " && cd $working_dir"
                    . " && $view_command"
                    . " && rm -rf $working_dir";
  $self->_log->debug( "Slurm command: |$slurm_command|" );
  ...
  my $memory_resource = "--mem-per-cpu=" . $job_spec->{memory};
  $self->_log->debug( "memory resource string: |$memory_resource|" );

  $self->_log->debug( "submitting Slurm job" );
