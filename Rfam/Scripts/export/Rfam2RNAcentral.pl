...
my $query2 = qq( select rfam_acc, rfam_id, f.type, 'seed', f.description, rs.ncbi_id, fr.rfamseq_acc, fr.seq_start, fr.seq_end, '', group_concat(distinct concat(dl.db_id,":",dl.db_link)) as dbxrefs, group_concat(distinct concat("PMID:",fl.pmid)) as PMIDS, rs.version, tx.species, tx.tax_string 
